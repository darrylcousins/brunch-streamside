...
const getRemoveFields = async (delivered) => {
  ...
  const { error, json } = await PostFetch({ src: `/api/order-sources`, data: { delivered }, headers })
    .then((result) => result)
    .catch((e) => ({
      error: e, json: null
    }));
 ...
 */
async function* RemoveOrdersModal(props) {
...
export default FormModalWrapper(RemoveOrdersModal, options);
