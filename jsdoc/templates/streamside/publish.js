  ...
  if (items && items.length) {
    items.forEach(function (item) {
      ...
      if (!hasOwnProp.call(item, 'longname')) {
        ...
        var itemName = item.name.replace(/^module:/, '');
        var itemLink = linktoFn(item.longname, itemName);
        //console.log(itemLink.match(/(<([^>]+)>)/gi), itemName.split('/'));
        var nameParts = itemName.split('/');
        var tagParts = itemLink.match(/(<([^>]+)>)/gi);
        var finalLink;
        if (nameParts.length) {
          if (nameParts.length === 2) {
            const [first, ...remainder] = nameParts;
            finalLink = `${tagParts[0]}<span class="${first}">${first}</span>/${remainder.join('/')}${tagParts[1]}`;
          } else if (nameParts.length === 3) {
            const [first, second, ...remainder] = nameParts;
            finalLink = `${tagParts[0]}<span class="${first}">${first}</span><span class="${second}">/${second}</span>/${remainder.join('/')}${tagParts[1]}`;
          }
        }
        itemsNav += '<li id="' + item.name.replace('/', '_') + '-nav">' + finalLink;
        if (methods.length) {
          itemsNav += "<ul class='methods' style='display:none'>";
